// splash screen text
const String myAppName = "SPLITWISE PRO";
const String myAppTagLine  = "Split bills, manage expenses";

//onboarding page text
const String myOnBoardingTitle = "Split Bills";
const String myOnBoardingSubtitle = "lorem ipsum";
const String myOnBoardingCounter = "1/3";
const String myOnBoardingTitle2 = "Track expenses";
const String myOnBoardingSubtitle2 = "lorem ipsum";
const String myOnBoardingCounter2 = "2/3";
const String myOnBoardingTitle3 = "Split Bills";
const String myOnBoardingSubtitle3 = "lorem ipsum";
const String myOnBoardingCounter3 = "3/3";

//welcome screen
const String myWelcomeTitle = "Split bills";
const String myWelcomeSubtitle = "Keep track of your shared expenses and balances with housemates, trips, groups, friends, and family.";

//login screen
const String myLoginTitle = "Welcome back";
const String myLoginSubtitle = "Make it work";
const String myRememberMe = "Remember me?";
const String myAlreadyHaveAccount = "Already have an Account? ";

//signup screen
const String mySignupTitle = "Get on board!";
const String mySignupSubtitle = "Create your profile to start your journey with us...";
const String mySignupAlreadyAccount = "Already have an account? ";

//random
const String myEmail = "Email";
const String myPassword = "Password";
const String myForgetPassword = "Forgot Password?";
const String mySignInWithGoogle = "Sign-in with Google";
const String myLogin = "LOGIN";
const String mySignup = "Signup";
const String myFullName = "Full Name";
const String myPhoneNo = "Phone No.";
